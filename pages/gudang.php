...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4" id="unitGrid">
  <?php if (empty($units)): ?>
    <p class="col-span-full text-center text-white/30 text-sm py-8">Belum ada data unit gudang, atau koneksi database gagal.</p>
  <?php endif; ?>
        ...
        <div class="flex gap-2">
          ...
          <?php if ($isKosong): ?>
            <form method="POST" action="unit_delete.php" onsubmit="return confirm('Hapus unit <?= htmlspecialchars(addslashes($u['kode'])) ?>?');" class="flex-1">
              ...
              <input type="hidden" name="redirect_to" value="dashboard.php?page=gudang"/>
