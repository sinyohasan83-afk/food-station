...
if ($pdo) {
    try {
        ...
        $countPengajuan = (int)$pdo->query("SELECT COUNT(*) c FROM booking_requests WHERE status = 'Menunggu'")->fetch()['c'];
        $countUnreadPesan = (int)$pdo->query("SELECT COUNT(*) c FROM pesan WHERE pengirim = 'tenant' AND is_read = 0")->fetch()['c'];
...
<aside class="sidebar w-64 flex-shrink-0 flex flex-col gap-1 overflow-y-auto p-4" id="sidebar">
  <a href="dashboard.php?page=home" class="sidebar-link <?= $currentPage === 'home' ? 'active' : '' ?>">
    <span class="sidebar-icon bg-indigo-500/20 text-indigo-400">
  ...
  <a href="dashboard.php?page=pengajuan" class="sidebar-link <?= $currentPage === 'pengajuan' ? 'active' : '' ?>">
    <span class="sidebar-icon bg-amber-500/20 text-amber-400">
      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
    </span>
    <span>Pengajuan Sewa</span>
    <?php if ($countPengajuan > 0): ?>
      <span class="ml-auto sidebar-count bg-amber-500/20 text-amber-400"><?= $countPengajuan ?></span>
    <?php endif; ?>
  </a>

  <a href="dashboard.php?page=pesan" class="sidebar-link <?= $currentPage === 'pesan' ? 'active' : '' ?>">
  ...
  <a href="dashboard.php?page=rekening" class="sidebar-link <?= $currentPage === 'rekening' ? 'active' : '' ?>">
    <span class="sidebar-icon bg-cyan-500/20 text-cyan-400">
  ...
  <?php if (($_SESSION['user_role'] ?? '') === 'superadmin'): ?>
    ...
    <a href="dashboard.php?page=pengaturan" class="sidebar-link <?= $currentPage === 'pengaturan' ? 'active' : '' ?>">
      <span class="sidebar-icon bg-slate-500/20 text-slate-300">
